...
function addLoadCard(type, index = null, loadCaseOrValues = 'G', valuesOrUserAdded = {}, userAddedArg = false) {
  ...
  card.innerHTML = `<div class="load-entry-head"><div class="load-entry-title"><span class="tag">${cfg.prefix}${rowIndex + 1}</span><span>${esc(cfg.title)}</span></div>${rowIndex || userAdded ? `<button type="button" class="btn load-remove-btn" aria-label="Remove ${cfg.prefix}${rowIndex + 1}"><i class="bi bi-trash"></i></button>` : ''}</div>
    <div class="load-entry-fields"><div class="load-field load-direction-field"><label>Direction<small>Y / Z</small></label><select data-load-type="${type}" data-field="direction" aria-label="${esc(cfg.title)} direction"><option value="Z"${direction === 'Z' ? ' selected' : ''}>Z direction</option><option value="Y"${direction === 'Y' ? ' selected' : ''}>Y direction</option></select></div><div class="load-field load-case-field"><label>Load case<small>G / Q1 / Q2</small></label><select data-load-type="${type}" data-field="loadCase" aria-label="${esc(cfg.title)} load case G Q1 Q2"><option value="G"${loadCase === 'G' ? ' selected' : ''}>G permanent</option><option value="Q1"${loadCase === 'Q1' ? ' selected' : ''}>Q1 main variable</option><option value="Q2"${loadCase === 'Q2' ? ' selected' : ''}>Q2 secondary variable</option></select></div>${cfg.fields.map(([field, label]) => {
...
function addAxialLoadRow(index = null, values = {}, userAdded = false) {
  ...
  row.innerHTML = `<div class="load-entry-head"><div class="load-entry-title"><span class="tag">N${rowIndex + 1}</span><span>Axial force</span></div>${rowIndex || userAdded ? `<button type="button" class="btn load-remove-btn" aria-label="Remove N${rowIndex + 1}"><i class="bi bi-trash"></i></button>` : ''}</div>
    <div class="load-entry-fields">
      <div class="load-field load-case-field"><label>Load case<small>G / Q1 / Q2</small></label><select data-axial-field="loadCase" aria-label="Axial force load case G Q1 Q2"><option value="G"${loadCase === 'G' ? ' selected' : ''}>G permanent</option><option value="Q1"${loadCase === 'Q1' ? ' selected' : ''}>Q1 main variable</option><option value="Q2"${loadCase === 'Q2' ? ' selected' : ''}>Q2 secondary variable</option></select></div>
      <div class="load-field axial-kind-field"><label>Type<small>X-axis</small></label><select data-axial-field="forceType" aria-label="Axial force type"><option value="compression"${forceType === 'compression' ? ' selected' : ''}>Compression</option><option value="tension"${forceType === 'tension' ? ' selected' : ''}>Tension</option></select></div>
